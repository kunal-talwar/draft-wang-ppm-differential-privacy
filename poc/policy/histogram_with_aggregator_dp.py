...
class HistogramWithAggregatorDp(DpPolicy):
    ...
    def __init__(self, epsilon: float, delta: float):
        ...
        # Changing one-hot vector to another will affect two coordinates, so
        # the change in L2-sensitivity is `sqrt(1 + 1) = sqrt(2)`.
        dgauss_sigma = agm.calibrateAnalyticGaussianMechanism(
            epsilon, delta, math.sqrt(2.0)
        )
        self.dgauss_mechanism = DiscreteGaussianWithZeroMean(dgauss_sigma)

    def add_noise_to_agg_share(self,
                               agg_share: AggShare,
                               ) -> AggShare:
        """
        Sample discrete Gaussian noise, and merge it with aggregate share.
        """
    ...
    def debias_agg_result(self,
        ...
        # fully implemented discrete Gaussian sampler (#10).
        return agg_result
